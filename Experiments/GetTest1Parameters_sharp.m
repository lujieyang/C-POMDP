...
%   See also TestOne, TestRep, POMDP, GetHallwayParameters.
  ...
  % Define the POMDP
    ...
    om{3}=GMixture(ones(1,4),...
                   {Gaussian(-11,sd) Gaussian( -5,sd) Gaussian(  3,sd) Gaussian(  9,sd)}); % door
    sc = 1;
    om{4}=GMixture(ones(1,8),...
                   {Gaussian(-9,sc) Gaussian(-7,sc) Gaussian(-3,sc) Gaussian(-1,sc) ...
                    Gaussian( 1,sc) Gaussian( 5,sc) Gaussian( 7,sc) Gaussian(11,sc)}); % corridor
    ...
    rm{3}=GMixture([-5 5 -5],{Gaussian(-25,250) Gaussian(3,1) Gaussian(25,250)});
